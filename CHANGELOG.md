...
## Следующая публикация — Привязка к мощной видеокарте на лэптопах, pagefile перестал убирать всё с диска C, распухание героев по шаблону + бэкап Ironman-сейвов, умнее советы при крахе на ландшафте, отдельная карточка под регрессии драйвера AMD RX 9000
...
- pagefile теперь оставляет маленькую запись на C: вместо того чтобы переносить всё на другой диск (для дампов памяти ядра);
