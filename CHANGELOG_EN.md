...
## 2026-07-01 — Mod version visible in the launcher, the mod no longer "scares" the game with updates + session-teardown crash-to-desktop guard

- **The mod version moves with every build again and is visible in the launcher.** The displayed version used to be frozen at 1.7.0.0, so you couldn't tell from the launcher which build you had. Now the build number moves on every update (currently 1.7.5.6) and shows both in the launcher and in the Doctor's window title.
- **Crash Doctor no longer appears in the game's "modules changed / version mismatch" load prompt.** Because the version now changes, loading an older save would make the game dutifully check Crash Doctor's version and complain. The new "Hide Crash Doctor's own version from the save-modlist warning" fix (Crash Fixes tab, on by default) removes Crash Doctor's own entries from that prompt — both on a version change and when the mod is added/removed mid-campaign. This is completely safe: Crash Doctor stores nothing in your save, so a version change can never affect loading. Every OTHER mod is still checked normally. Works on Bannerlord 1.2 / 1.3 / 1.4.
- **Crash-to-desktop guard while the game is closing the session (after a siege/battle).** A crash (`NullReferenceException` in `ScreenManager.CleanScreens`) while the game tears the whole session down: `MBGameManager.EndGame` → `GameStateManager.CleanStates` → `CleanScreens`. Seen after a successful assault on a besieger camp where the siege breaks ("besiegers -0") and the game is forced back to the main menu — while the screen stack is cleaned an already-destroyed screen is dereferenced, and instead of a soft exit to the menu the game crashes to desktop. The new "Session-teardown crash-to-desktop guard (return to menu)" fix on the Crash Fixes tab (on by default) swallows that teardown error: you land on the main menu (your last save is intact, just load it) instead of the desktop, and get a dialog explaining what happened. This is a **symptom** guard — the primary fault is the broken siege event, which isn't in this crash; if the game keeps force-exiting at the same siege, send the report from the Crash-diagnosis tab. Works on Bannerlord 1.2 / 1.3 / 1.4.
